...
# funciones que me ayudaran con validaciones adicionales
...
class RegistroSerializer(SQLAlchemyAutoSchema):
    ...
    correo = auto_field(validate=validate.Email(
        error='El correo no cumple con el formato correcto'))
...
class CambiarPasswordSerializer(Schema):
    ...
    passwordNueva = fields.String(required=True, validate=validate.Regexp(
...
class ConfirmarResetPasswordSerializer(Schema):
    token = fields.String(required=True)
    nuevaPassword = fields.String(required=True, validate=validate.Regexp(
        '^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@#$%^&?!])[A-Za-z\d@#$%^&?!]{6,}$'))
